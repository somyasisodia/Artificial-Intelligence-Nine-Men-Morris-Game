...
public class MiniMaxGame {
	...
	public static void main(String[] args) throws IOException{
		//int depth = 3;
		MiniMaxGame ob = new MiniMaxGame();
		//Board bro = new Board();
		maxStep = 50;
		...
		Board b = new Board();
		System.out.println("Playing MiniMax Game ");
		//root.setboard(sChar);
	...
	public static void insertChild(TreeNode node, int depth){
		...
		if(depth>0){
			...
			if(numBlackPieces(node.board) <= 2)
				...
				node.staticValue = -100000;
			else if(numBlackMoves(node.board) == 0)
				node.staticValue = 1000000;
			else
				node.staticValue = ((1000 * (numWhitePieces(node.board) - numBlackPieces(node.board))) - numBlackMoves(node.board));
		}
	}
